...
export type PartnerPaymentAccessParams = {
...
const TEMPLATE_PATH = path.join(
  process.cwd(),
  "src/lib/email-templates/partner-payment-access.html"
);
...
export function partnerPaymentAccessEmailHtml(params: PartnerPaymentAccessParams) {
  ...
  const paymentReferenceBlock = paymentReference
    ? fillTemplate(extractSection(source, "payment-reference"), {
  ...
  return fillTemplate(getMainTemplate(source), {
    ...
    loginEmail: escapeHtml(params.loginEmail),
    temporaryPassword: escapeHtml(params.temporaryPassword),
    loginUrl: escapeHtml(params.loginUrl),
    additionalServicesUrl: escapeHtml(params.additionalServicesUrl),
    paymentReferenceBlock,
    year: String(new Date().getFullYear()),
  }).trim();
}

export function partnerPaymentAccessEmailSubject(eventTitle: string) {
  return `Booth payment confirmed — ${eventTitle} | ${BRAND.name}`;
}
